...
#include "HolonomicRRTNode.h"
#include "dummyClasses.h"
#include <vector>
#include "4Vector.h"
#include "7Vector.h"

class HolonomicRRT : public CHolonomicPathPlanning
{
public:
  HolonomicRRT(int theStartDummyID,int theGoalDummyID,
...
private:
  ...
  float distance(HolonomicRRTNode* a, HolonomicRRTNode* b);
  int getVector(HolonomicRRTNode* fromPoint,HolonomicRRTNode* toPoint,float vect[7],float e,float& artificialLength,bool dontDivide);
  std::vector<HolonomicRRTNode*> getNearNeighborNodes(std::vector<HolonomicRRTNode*>& nodes, HolonomicRRTNode* node, float radius);
  HolonomicRRTNode* getClosestNode(std::vector<HolonomicRRTNode*>& nodes, HolonomicRRTNode* sample);
  HolonomicRRTNode* extend(HolonomicRRTNode* from, HolonomicRRTNode* to,
                               bool shouldBeConnected, CDummyDummy* dummy, float &artificialCost);
  HolonomicRRTNode* slerp(HolonomicRRTNode*, HolonomicRRTNode*, float t);
  bool gotPotential(HolonomicRRTNode* it);
