...
HolonomicRRTstarNode::HolonomicRRTstarNode(const C4Vector& rotAxisRot, const C4Vector& rotAxisRotInv)
  : CHolonomicPathNode(rotAxisRot, rotAxisRotInv) {
	_cost = 0.0;
}

HolonomicRRTstarNode::HolonomicRRTstarNode(int theType, const C7Vector& conf, const C4Vector& rotAxisRot, const C4Vector& rotAxisRotInv)
  : CHolonomicPathNode(theType, conf, rotAxisRot, rotAxisRotInv) {
  _cost = 0.0;
}

HolonomicRRTstarNode::HolonomicRRTstarNode(int theType, float searchMin[4], float searchRange[4],
...
HolonomicRRTstarNode::~HolonomicRRTstarNode() {
}

HolonomicRRTstarNode* HolonomicRRTstarNode::copyYourself() {
  HolonomicRRTstarNode* newNode = new HolonomicRRTstarNode(_rotAxisRot, _rotAxisRotInv);
...
void HolonomicRRTstarNode::removeChild(HolonomicRRTstarNode *child) {
  bool flag = false;
  for (int i = 0; i < int(_children.size()); i++) {
    if (_children[i] == child) { // If the order of children doesn't matter, it's faster vector::erase()
...
void HolonomicRRTstarNode::updateChildrenCosts(float delta_cost) {
  for (int i = 0; i < int(_children.size()); i++) {
