...
#include "HolonomicPRMNode.h"
#include "dummyClasses.h"
#include <vector>
#include "4Vector.h"
#include "7Vector.h"

class HolonomicPRM : public CHolonomicPathPlanning
{
public:
  HolonomicPRM(int theStartDummyID,int theGoalDummyID,
...
private:
  ...
  float distance(HolonomicPRMNode* a, HolonomicPRMNode* b);
  int getVector(HolonomicPRMNode* fromPoint,HolonomicPRMNode* toPoint,float vect[7],float e,float& artificialLength,bool dontDivide);
  std::vector<HolonomicPRMNode*> getNearNeighborNodes(std::vector<HolonomicPRMNode*>& nodes, HolonomicPRMNode* node, float radius);
  HolonomicPRMNode* getClosestNode(std::vector<HolonomicPRMNode*>& nodes, HolonomicPRMNode* sample);
  HolonomicPRMNode* extend(HolonomicPRMNode* from, HolonomicPRMNode* to,
                               bool shouldBeConnected, CDummyDummy* dummy, float &artificialCost);
  HolonomicPRMNode* slerp(HolonomicPRMNode*, HolonomicPRMNode*, float t);
  bool gotPotential(HolonomicPRMNode* it);
