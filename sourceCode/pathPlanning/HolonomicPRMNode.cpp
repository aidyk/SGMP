...
HolonomicPRMNode::HolonomicPRMNode(const C4Vector& rotAxisRot, const C4Vector& rotAxisRotInv)
  : CHolonomicPathNode(rotAxisRot, rotAxisRotInv) {
	_cost = 0.0;
}

HolonomicPRMNode::HolonomicPRMNode(int theType, const C7Vector& conf, const C4Vector& rotAxisRot, const C4Vector& rotAxisRotInv)
  : CHolonomicPathNode(theType, conf, rotAxisRot, rotAxisRotInv) {
  _cost = 0.0;
}

HolonomicPRMNode::HolonomicPRMNode(int theType, float searchMin[4], float searchRange[4],
...
HolonomicPRMNode::~HolonomicPRMNode() {
}

HolonomicPRMNode* HolonomicPRMNode::copyYourself() {
  HolonomicPRMNode* newNode = new HolonomicPRMNode(_rotAxisRot, _rotAxisRotInv);
...
void HolonomicPRMNode::removeNode(HolonomicPRMNode *node) {
  bool flag = false;
  for (int i = 0; i < int(_nodes.size()); i++) {
    if (_nodes[i] == node) { // If the order of nodes doesn't matter, it's faster vector::erase()
