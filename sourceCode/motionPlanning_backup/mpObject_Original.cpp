...
float* CmpObject::findPath(const float* startConfig,const float* goalConfig,int options,float stepSize,int* outputConfigsCnt,int maxTimeInMs,const int* auxIntParams,const float* auxFloatParams)
{
	...
	Node* startN=new Node(_cyclicJointFlags.size(),startConfig,tipStartTransf);
	_nodeListFromStart.push_back(startN);
	Node* goalN=new Node(_cyclicJointFlags.size(),goalConfig,tipGoalTransf);
	...
	{ // ok, the start and goal configs are far enough
		std::vector<Node*>* current=&_nodeListFromStart;
		std::vector<Node*>* nextCurrent=&_nodeListFromGoal;
		int startTime=simGetSystemTimeInMs(-1);
		bool firstPass=true;
		while ((maxTimeInMs==0)||(_simGetTimeDiffInMs(startTime)<maxTimeInMs))
		{
			for (int altern=0;altern<2;altern++)
			{
				...
				{ // if first pass, we try to link start to goal directly!
					randNode=new Node(_cyclicJointFlags.size(),goalN->jointPositions,goalN->tipTransf);
				}
				else
				{ // we have to create a random node:
					...
					randNode=new Node(_cyclicJointFlags.size(),&pos[0],tipStartTransf); // last argument is not used
				}
				if (activityToConsole)
					printf("*");
				Node* closest=_getPhase2ClosestNode(*current,randNode);
				...
				{ // ok, we could extend the node a bit
					...
					Node* closestConnect=_getPhase2ClosestNode(*nextCurrent,closest);
				...
				delete randNode;
				std::vector<Node*>* tmpCurrent=nextCurrent;
				nextCurrent=current;
				current=tmpCurrent;
				if (foundAPath)
					break;
...
Node* CmpObject::_extendNode(std::vector<Node*>& nodeList,Node* toBeExtendedNode,Node* goalNode,float stepSize,bool connect,int collisionCheckMask)
{	// return value is !=NULL if extension was performed and connect is false
	...
	for (int currentPass=0;currentPass<passes;currentPass++)
	{
		...
		Node* newNode=new Node(_cyclicJointFlags.size(),&jointPos[0],tipTransf);
...
int CmpObject::_getPhase2Vector(const Node* startNode,const Node* goalNode,float stepSize,float* returnVector)
